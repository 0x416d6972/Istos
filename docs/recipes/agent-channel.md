...
mesh = Istos(
    config=IstosZenohConfig(mode="client", connect=["tcp/router:7447"]),
...
async def chat_ws(ws: WebSocket):
    await ws.accept()
    # Prefer Authorization from a prior HTTP login cookie / query param —
    # browsers cannot set WS headers. Pass token= into open_channel when you have one.
    chan = await mesh.open_channel("agent/chat")
    try:
